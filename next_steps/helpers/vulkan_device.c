...
static int scoreDeviceSuitability(struct vulkan_cfg *cfg,
  ...
  for (size_t i = 0, match = false; i < cfg->_requiredDeviceExtensions_n; i++) {
    for (size_t j = 0; j < deviceSupportedExtensions_n; j++) {
      if (!strcmp(cfg->_requiredDeviceExtensions[i],
...
int pickPhysicalDevice(struct vulkan_cfg *cfg) {
  ...
  for (size_t i = 0; i < physicalDevices_n; i++) {
    ...
    score = scoreDeviceSuitability(cfg, &physicalDevices[i]);
    if (score > bestScore) {
...
int createLogicalDevice(struct vulkan_cfg *cfg) {
  ...
  for (size_t i = 0; i < cfg->_requiredDeviceExtensions_n; i++) {
    printf("Enabling device extension: %s\n",
           cfg->_requiredDeviceExtensions[i]);
  }
  ...
  VkDeviceCreateInfo deviceCreateInfo = {VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO,
                                         ...
                                         0,    // Deprecated
                                         NULL, // Deprecated
                                         cfg->_requiredDeviceExtensions_n,
