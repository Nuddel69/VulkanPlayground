...
int createVulkanInstance(struct vulkan_cfg *cfg) {
  ...
  cfg->_requiredInstanceExtensions =
      realloc(cfg->_requiredInstanceExtensions,
              (requiredExtensions_n + cfg->_requiredInstanceExtensions_n) *
                  sizeof(char *));

  for (size_t i = 0; i < requiredExtensions_n; i++) {
    cfg->_requiredInstanceExtensions[cfg->_requiredInstanceExtensions_n + i] =
        requiredExtensions[i];
  }

  cfg->_requiredInstanceExtensions_n += requiredExtensions_n;
  ...
  printf("Required Extension Count: (%d)\n",
         cfg->_requiredInstanceExtensions_n);

  for (size_t i = 0, match = 0; i < cfg->_requiredInstanceExtensions_n; i++) {
    match = 0;
    const char *extension = cfg->_requiredInstanceExtensions[i];
