...
static void transitionImageLayout(struct vulkan_cfg *cfg, uint32_t imageIndex,
                                  uint32_t frameIndex, VkImageLayout old_layout,
  ...
  vkCmdPipelineBarrier2(cfg->_cmd_buffers[frameIndex], &dependencyInfo);
}
...
int createCommandBuffers(struct vulkan_cfg *cfg) {
  ...
  // cfg->_cmd_buffers = malloc(cfg->_cmd_buffers_n * sizeof(VkCommandBuffer));
  printf("Commandbuffer count: (%d)\tPointer to first: (%x)",
         cfg->_cmd_buffers_n, cfg->_cmd_buffers);

  VkCommandBufferAllocateInfo bufferInfo = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
      .pNext = NULL,
      .commandBufferCount = cfg->_cmd_buffers_n,
      .commandPool = cfg->_cmd_pool,
      .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
  };

  status =
      vkAllocateCommandBuffers(cfg->_device, &bufferInfo, cfg->_cmd_buffers);
...
int recordCommandBuffer(struct vulkan_cfg *cfg, uint32_t imageIndex,
  ...
  VkCommandBuffer cmd = cfg->_cmd_buffers[frameIndex];
  ...
  if (frameIndex > cfg->_cmd_buffers_n) {
    fprintf(stderr, "Invalid frame index\n");
    return -1;
  }

  // printf("Starting command buffer\n");
  status = vkBeginCommandBuffer(cfg->_cmd_buffers[frameIndex], &beginInfo);
  ...
  transitionImageLayout(cfg, imageIndex, frameIndex, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  transitionImageLayout(
      cfg, imageIndex, frameIndex, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
